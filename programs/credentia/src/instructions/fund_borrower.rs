...
impl<'info> FundBorrower<'info>{
    ...
    pub fn fund_borrower(&mut self) -> Result<()>{
        ...
        require!(self.loan_account.status == LoanStatus::Requested , ErrorCode::LoanNotActive);
        require!( self.lender.lamports() >= self.loan_account.loan_amount  , ErrorCode::InsufficientBalance);
